...
TOOLS = [
    {
        ...
        "how": [
            ...
            "Alt+drag a box, Shift+drag a circle, Alt+Shift+drag freehand, or Space+drag+click a "
            "rotated box to multi-select points - works in every tool, not just Select. Hold Ctrl "
            "on top of any of these (or alone) to ADD to the selection instead of replacing it; a "
            "plain Ctrl-click toggles one point at a time.",
    ...
    },
    {
        ...
        "controls": [
            ...
            ("detect custom field", "on by default; when Courseplay is active, also finds ground "
                                     ...
                                     "boundary recorded with Courseplay's custom-field tool, not "
            ...
            ("multi", "off by default. On, each click stages another scanned region instead of "
                      "placing it, and right-click places all of them at once, bridging the ones "
                      "that ended up close together (bridge distance sets how close). For a field "
                      "a lane splits into disconnected tilled patches, since a live scan can only "
                      "ever return the one piece the click landed on."),
        ],
    },
    {
    ...
    },
    {
        ...
        "summary": "Drag a waypoint, a run, or a span - move it, copy it, break it off, or slide "
                   "it sideways.",
        "what": "Move drags one or more waypoints to a new spot and re-seats them on the real "
                ...
                "creates the move as a new, disconnected piece instead; break (copy's own "
                "sub-option) goes further and deletes the originals too, so the piece ends up "
                "relocated and cleanly detached rather than stretching a connection across the "
                "map. Offset slides an existing run or span sideways IN PLACE - unlike Parallel, "
                "it does not create a new track, the same waypoints just move over, connections "
                "and all. Auto-hookup, if turned on, silently reconnects a dangling end (one made "
                "by a break, a copy, or just an ordinary move) to whatever it lands near.",
        "how": [
            ...
            "A ctrl-click / box (Alt+drag) / circle (Shift+drag) / freehand (Alt+Shift+drag) / "
            "rotated box (Space+drag+click) selection, if one exists, always wins over picks - "
            "grab any point IN it to drag the whole set, rigid.",
            "\"copy\" (hotkey B) toggles whether the drag creates a new, disconnected copy instead "
            "of moving the originals. It can be turned on before, during, or even just after a "
            "drag - toggling it on right after releasing still converts that move into a copy.",
            "\"disconnect\" severs the moved piece's outside connections. On its own (copy off) it "
            "just moves and cuts loose, no clone. With copy also on, it additionally deletes the "
            "originals once the copy is made - the piece ends up relocated, not duplicated.",
            "On Run or Span, \"offset\" changes what a drag does: instead of a free 2D move, the "
            "cursor's distance from the chain slides it sideways, the whole thing at once. "
            "Releasing does NOT drop it - wheel (or type a value) to fine-tune the distance, then "
            "right-click to finish. A stray click elsewhere is ignored while one is pending, and "
            "the wheel adjusts the pending offset from anywhere in the view, not just over the card.",
            "\"offset falloff\", when offset is on, tapers the slide toward either end of the "
            "chain instead of sliding it all the same amount - useful for blending an offset "
            "smoothly into an existing junction or track instead of leaving a hard kink.",
            "\"auto-hookup\", if on, checks any dangling end left by the drag (at most one "
            "existing connection) against nearby waypoints and wires it up automatically, no "
            "confirmation - its own distance/divergence tolerances appear on the card while it is on.",
            "Hold R and scroll the wheel WHILE dragging to rotate whatever is moving, "
            "live - around either the point you grabbed or the moved set's centroid, whichever "
            "\"rotate pivot\" is set to.",
            "The tool card auto-hides while you drag, so it is never in the way.",
        ],
        "controls": [
            ("picks", "what a plain drag grabs: Point, Run, or Span. A box/circle/freehand/"
                      "rotated-box/ctrl-click selection always overrides picks when you grab a "
                      "member of it."),
            ...
            ("disconnect", "on/off; severs outside connections. With copy off, moves in place and "
                           "cuts loose. With copy on, also deletes the originals once copied."),
            ("offset", "on/off (Run/Span only); drag-then-wheel sideways slide, right-click to finish."),
            ("rotate pivot", "click point or centroid - what R+wheel rotates the dragged "
                             "set around. Takes effect on the NEXT grab."),
            ("offset falloff", "on/off (while offset is on); tapers the slide near the chain's "
                               "own two ends instead of sliding it all rigidly."),
...
GENERAL = {
    ...
    "sections": [
        ...
        ("Mouse & camera", [
            ...
            "Ctrl - the universal \"add to selection\" modifier: held on top of any shape below, or "
            "alone (plain Ctrl-click) to toggle a single point. Works in every tool.",
            ...
            "Space+drag+click - rotated box-select: hold Space, press-drag-release sets one edge "
            "(Space only needs to be held at the press), then one more plain click sets the width "
            "and finishes.",
        ]),
        ("The tool card", [

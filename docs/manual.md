...
- **detect custom field** — on by default; when Courseplay is active, also finds ground plowed to connect two separate map fields into one, not just a field shipped with the map. No effect without Courseplay. Turn it off to fall back to the map-field-only lookup if it ever misreads a field on your save.

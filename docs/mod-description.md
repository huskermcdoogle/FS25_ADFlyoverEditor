...
- **Field loop** — Generate a complete, drivable two-way loop just outside a field's boundary, smoothed to a turning radius and nudged inward around any trees in the way. It's a standalone loop — no map markers, not wired to the rest of the network — so you connect it in afterwards. Controls: margin, tree clearance, turning radius and vehicle height.

# Build log — companion bring-up (historical)

This is the staged bring-up record from the early (0.7.x) days of the companion mod, kept as history:
how each layer was verified against **stock** AutoDrive with the fork disabled, and the in-game
commands that proved it. Current status lives in the [README](../README.md) and in the
[releases](../../releases); the notes below are frozen at the build they were written for.
...
| straighten | RDP by tolerance, keeps real bends — **untested at the time** |
| parallel | track alongside a span — **untested at the time** |
| siding | parallel, splined in at both ends — **untested at the time** |
